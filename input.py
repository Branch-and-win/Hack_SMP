...
class ModelInput:
    def __init__(self, input_folder: str):
        ...
        self.edges_dict: Dict[int, int, Edge] = {}
    ...
    def read_ports_xlsx(self) -> None:
        ...
        for _, row in port_data.iterrows():
            port = Port(id = row['point_id'], name = row['point_name'], longitude = row['longitude'], latitude = row['latitude'])
    ...
    def read_vessels_xlsx(self) -> None:
        ...
        for _, row in vessel_data.iterrows():
            vessel = Vessel(id = row['vessel_id'], name = row['vessel_name'], is_icebreaker = False, 
                port_start = self.ports_dict[row['start_point_id']], port_end = self.ports_dict[row['end_point_id']],
                time_start = self.date_to_time(row['date_start']), max_speed = row['max_speed'], class_type = row['class_type'])
    ...
    def read_icebreakers_xlsx(self) -> None:
        ...
        for _, row in icebreaker_data.iterrows():
            icebreaker = Vessel(id = row['vessel_id'], name = row['vessel_name'], is_icebreaker = True, 
                port_start = self.ports_dict[row['start_point_id']], port_end = None,
                time_start = 0, max_speed = row['max_speed'], class_type = row['class_type'])
    ...
    def read_edges_xlsx(self) -> None:
        ...
        for _, row in edge_data.iterrows():
            edge_1 = Edge(port_from = self.ports_dict[row['start_point_id']], port_to = self.ports_dict[row['end_point_id']], 
                distance = row['length'])
            edge_2 = Edge(port_from = self.ports_dict[row['end_point_id']], port_to = self.ports_dict[row['start_point_id']], 
                distance = row['length'])
        ...
        for p in self.ports:
            edge = Edge(port_from = p, port_to = p, distance = 0)
            ...
            for e in self.edges:
                for t in self.times:
                    ...
                    if t + (1 if e.port_from.id == e.port_to.id else round(e.distance / 15, 0)) in self.times:
                        departure = Departure(vessel = v, edge = e, time = t)
    ...
    def generate_locations(self):
        for v in self.vessels:
            for p in self.ports:
                for t in self.times:
                    location = Location(vessel = v, port = p, time = t)
